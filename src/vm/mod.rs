...
mod tests {
    ...
    use crate::vm::internals::{Instruction, Procedure};
    ...
    fn simple_fibonacci() {
        let mut program = Vec::new();
        program.push(Instruction::new(Procedure::Push, 0)); // f(0)
        program.push(Instruction::new(Procedure::Push, 1)); // f(1)

        for _ in 0..9 {
            program.push(Instruction::new(Procedure::Swap, 0));
            program.push(Instruction::new(Procedure::Over, 0));
            program.push(Instruction::new(Procedure::Add, 0));
        }
